...
export default function ManageCompletions() {
  ...
  const fetchData = async () => {
    try {
      // Fetch completions, family members, and tasks
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 mt-4">Loading completions...</p>
      </div>
    )
  }
            ...
            .map(([date, dayCompletions]) => (
              <div key={date} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="bg-gray-50 px-6 py-3 border-b border-gray-200">
                  ...
                  <p className="text-sm text-gray-600">{dayCompletions.length} completions</p>
                </div>
